...
export const animateWords = ({ id, els, ...props }) => {
	...
	GS.set(splitted.words, { y: 60, autoAlpha: 0 })

	const __TL = GS.timeline({
		id,
...
export const animateLines = ({ id, els, ...props }) => {
	...
	GS.set(splitted.lines, { y: 60, autoAlpha: 0 })
...
export const revealImgs = ({ id, els, ...props }) => {
	...
	const { wraps, imgs } = els.reduce(
		(acc, el) => {
			...
			if (!wrap.isConnected) {
				...
				wrap.classList.add(props?.color ?? 'bg-indigo-700')
	...
	GS.set([...imgs, ...wraps], { scaleX: 0, transformOrigin: 'left' })

	const tween = GS.timeline({
		id,
